...
class AdminCreateQuestionView(ContextTitleMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['suggested_order'] = Question.objects.filter(survey=self.survey).count() + 1
        return context
...
class AdminUpdateQuestionView(ContextTitleMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['type_field_id'] = self.type_field_id
        context['current_order'] = self.get_object().ordering
        return context
...
@method_decorator(staff_member_required, name='dispatch')
class AdminSurveyQuestionsView(ContextTitleMixin, DetailView):
    model = Survey
    template_name = 'djf_surveys/admins/survey_questions.html'
    title_page = _("Manage Questions")
    slug_url_kwarg = 'survey_slug'
