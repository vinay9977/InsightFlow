...
class Survey(BaseModel):
    ...
    valid_until = models.DateTimeField(_("valid until"), blank=True, null=True,
                                       help_text=_("Specify the expiration date for this survey."))
    ...
    start_date = models.DateTimeField(
        ...
        help_text=_("Date and time when the survey becomes available. Leave blank for immediate availability.")
    )
    end_date = models.DateTimeField(
        _("end date"), blank=True, null=True,
        help_text=_("Date and time when the survey expires. Leave blank for no expiration.")
    )
    target_responses = models.PositiveIntegerField(
        _("target responses"), 
        default=100,
        help_text=_("Target number of responses for this survey")
    )

    @property
    def response_count_progress(self):
        """Get the current number of responses."""
        return UserAnswer.objects.filter(survey=self).count()
    
    @property
    def response_rate(self):
        """Calculate the response rate as a percentage."""
        if self.target_responses == 0:
            return 0
        return min(100, int((self.response_count_progress / self.target_responses) * 100))
