...
class SurveyListView(ContextTitleMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not show_all:
            now = timezone.now()
            # Show only surveys that are currently active (within schedule)
            filter_start = models.Q(start_date__isnull=True) | models.Q(start_date__lte=now)
            filter_end = models.Q(end_date__isnull=True) | models.Q(end_date__gt=now)
            filter_schedule = filter_start & filter_end
        
        query = self.request.GET.get('q')
        if query:
            object_list = self.model.objects.filter(name__icontains=query, **filter)
            if not show_all:
                object_list = object_list.filter(filter_schedule)
        else:
...
class CreateSurveyFormView(ContextTitleMixin, SurveyFormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Check if the survey is active based on schedule
        now = timezone.now()
        if survey.start_date and now < survey.start_date:
            messages.warning(request, gettext("This survey is not yet available. It will become available on %s.") % survey.start_date.strftime("%Y-%m-%d %H:%M"))
            return redirect("djf_surveys:index")
        if survey.end_date and now > survey.end_date:
            messages.warning(request, gettext("This survey has expired. It was available until %s.") % survey.end_date.strftime("%Y-%m-%d %H:%M"))
            return redirect("djf_surveys:index")
...
class DetailResultSurveyView(ContextTitleMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        # Check if user is admin or the owner of the survey response
        user_answer = self.get_object()
        if request.user.is_superuser or user_answer.user == request.user:
            return super().dispatch(request, *args, **kwargs)
        
        messages.warning(request, gettext("You don't have permission to access this page."))
        return redirect("djf_surveys:index")
